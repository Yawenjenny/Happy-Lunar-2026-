...
export const drawMountain = (ctx: CanvasRenderingContext2D, m: Mountain) => {
  ctx.save();
  ...
  // Since stone is now lighter, we make the highlight even lighter (White)
...
export const drawYuanbao = (ctx: CanvasRenderingContext2D, yb: Yuanbao) => {
  ctx.save();
...
export const drawClouds = (ctx: CanvasRenderingContext2D, frame: number, width: number) => {
  ctx.fillStyle = 'rgba(255, 255, 255, 0.4)';
  
  const cloudSpeed = 0.5;
  const offset1 = (frame * cloudSpeed) % (width + 100);
  const offset2 = ((frame * cloudSpeed * 0.8) + 400) % (width + 100);
